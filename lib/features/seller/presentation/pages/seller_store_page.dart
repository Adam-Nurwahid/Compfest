...
class _SellerStorePageState extends State<SellerStorePage> {
  ...
  void _saveStore(AppState appState, Store? currentStore) {
    ...
    final nameExists = dummyStores.any((s) {
      if (currentStore != null && s.id == currentStore.id) return false;
    ...
    if (nameExists) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nama Toko sudah digunakan! Silakan pilih nama lain.'),
          backgroundColor: AppColors.danger,
        ),
      );
      return;
    ...
    if (currentStore == null) {
      ...
      appState.updateStore(updatedStore);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final currentStore = appState.currentUserStore;
    ...
    final productsCount = dummyProducts.where((p) => p.storeId == currentStore.id).length;
